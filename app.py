...
def venues():
  ...
  for town in towns:
    ...
    # add the town +state as a tuples
      towns.add((venues.city, venue.state))
      new_shows = [show for show in venue.shows if show.start_time > datetime.now]
      if (venue.city == town[0]) and (venue.state == town[1]):
...
def search_venues():
  ...
  word_searched = request.form.get('search_term', '')
  word_queried = Venue.query.filter(Venue.name.ilike('%' + word_searched + '%')).count()
  response={
    "count": word_queried,
    "data": word_queried
  }
  return render_template('pages/search_venues.html', results=response, search_term=word_searched)
